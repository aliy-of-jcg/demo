...
import { useReturningAnalysisReturnIntervals } from '@/lib/hooks/returning-analysis/useReturnIntervals';

interface ReturnIntervalsSectionProps {
  startDate: string;
  endDate: string;
  enabled: boolean;
}

export function ReturnIntervalsSection({
  startDate,
  endDate,
  enabled,
}: ReturnIntervalsSectionProps) {
  const t = useTranslations('returningAnalysis');
  const { data, loading, error } = useReturningAnalysisReturnIntervals({
    startDate,
    endDate,
    enabled,
  });
