...
export default function DebugSessionsPage() {
  ...
  const [debugDomains, setDebugDomains] = useState<string[]>([]);
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      const response = await fetchWithAuth(`/api/analytics/debug-sessions?${params}`);
      const data = await response.json();

      if (data.success) {
        setSessions(data.sessions);
        setDebugDomains(data.debug_domains || []);
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold text-gray-900">DEBUG: Session Journeys</h1>
          ...
          <p className="text-gray-600 mb-2">
            Tracking sessions only from: <strong>{debugDomains.join(', ')}</strong>
          </p>
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800">
            ⚠️ This is a temporary debug page. Remove after debugging is complete.
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="text-sm text-gray-600">Total Sessions (Debug)</div>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl p-12 text-center shadow-sm border border-gray-200">
            ...
            <p className="text-gray-600">No sessions found for the tracked domains in the selected date range.</p>
            <p className="text-sm text-gray-500 mt-2">Tracking: {debugDomains.join(', ')}</p>
          </div>
        ) : (
          <div className="space-y-6">
            {sessions.map((session) => (
              <div
                ...
                className="bg-white rounded-xl shadow-sm border-2 border-red-200 overflow-hidden hover:shadow-md transition-shadow"
              >
                {/* Session Header */}
                <div className="bg-gradient-to-r from-red-50 to-orange-50 p-6 border-b border-red-200">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-medium text-red-600">
                      Domain: {session.tracked_domain || 'unknown'}
                    </span>
                  </div>
